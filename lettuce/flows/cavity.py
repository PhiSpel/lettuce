...
class Cavity2D(object):
    ...
    def initial_solution(self, x):
        return np.array([0 * x[0]], dtype=float), np.array([0 * x[0], 0 * x[1]], dtype=float)  # p, u
    ...
    def boundaries(self):
        ...
        return [
            ...
            EquilibriumBoundaryPU(top, self.units.lattice, self.units, np.array([1.0, 0.0])),
        ]
